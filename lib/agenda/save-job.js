...
module.exports = async function(job) {
  try {
    ...
    const disabled = job.attrs.disabled || false;
    const lockedAt = this._notLocked;
    ...
    if (props.type === 'single') {
      ...
      update.$setOnInsert = {...protect, lockedAt};

      // Initialise the disabled flag if it hasn't already been defined
    ...
    if (unique) {
      ...
      if (uniqueOpts && uniqueOpts.insertOnly) {
        update = {$setOnInsert: {...props, lockedAt}};
      } else {
        update.$setOnInsert = {lockedAt};
      }
